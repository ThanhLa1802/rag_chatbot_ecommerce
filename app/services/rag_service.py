...
from app.services.window_memory import get_search_cache
...
def retrieve_context(query: str, category: str = None, max_price: float = None, top_k: int = 4) -> str:
    ...
    Uses Redis semantic search cache to speed up retrieval.
    """
    try:
        # 1. Check semantic search cache first
        logger.info("category: %s, max_price: %s", category, max_price)
        cached_result = search_cache.get_cached_result(query, category, max_price)
        if cached_result:
            logger.info(f"✅ Cache HIT for query: {query[:50]}...")
            return cached_result["results"]
        
        logger.info(f"Cache miss, querying Qdrant...")
        
        # 2. Get or generate embedding
        cached_embedding = search_cache.get_cached_embedding(query)
        
        if cached_embedding:
            logger.info(f"Using cached embedding for query: {query[:50]}...")
            query_vector = cached_embedding
        else:
            # Nhúng câu hỏi của người dùng thành Vector
            embed_res = ai_client.embeddings.create(
                input=query, 
                model="text-embedding-3-small"
            )
            query_vector = embed_res.data[0].embedding
            logger.info(f"Generated new embedding for query: {query[:50]}...")

        # 3. Xây dựng bộ lọc Metadata (Pre-filtering)
        must_conditions = []
        ...
        # 4. Tìm kiếm Vector trên Qdrant (Chỉ quét các document thỏa mãn filter)
        ...
        # 5. Trích xuất nội dung text từ các chunk tìm được
        ...
        logger.debug(f"Qdrant returned {len(context_chunks)} chunks")
        ...
        # 6. Cache the result
        print(f"Caching search result for query: {query[:50]}...")
        search_cache.cache_search_result(
            query=query,
...
def generate_answer_stream(query: str, conversation_context: str = None, category: str = None, max_price: float = None):
    """
    Generator function: Gọi retrieve_context lấy dữ liệu, sau đó stream kết quả từ LLM về.
    
    Args:
        query: Original query (for cache lookups)
        conversation_context: Full context with conversation history (for LLM)
        category: Category filter
        max_price: Price filter
    """
    logger.info(f"🔍 Starting generate_answer_stream for query: {query[:50]}...")
    logger.debug(f"   Filters: category={category}, max_price={max_price}")
    
    # Bước 1: Rút trích ngữ cảnh từ Database (use original query for cache)
    logger.info("📚 Step 1: Retrieving context from Qdrant/Cache...")
    ...
    if not context:
        logger.warning("⚠️  No context found for query")
    ...
    logger.info("📝 Step 2: Building system prompt...")
    ...
    # Use conversation context if provided, otherwise use original query
    user_message = conversation_context if conversation_context else query
    
    # Bước 3: Gọi API OpenAI với chế độ Streaming
    logger.info("🤖 Step 3: Calling OpenAI API with streaming...")
    try:
        response = ai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
        ...
        logger.info("✅ OpenAI API connection established, starting streaming...")
        chunk_count = 0
